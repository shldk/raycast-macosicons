...
export async function restoreDefaultIcon(appPath: string) {
  await execPromise(
    `xattr -d -r com.apple.FinderInfo "${appPath}" && rm -rf "${appPath}"$'/Icon\r' && killall Dock`,
  );
  return await History.removeApplication(appPath);
}

export async function setIcon(appPath: string, imagePath?: string) {
  if (!imagePath) {
    return;
  }
...
export async function getApplications() {
  const { stdout } = await execPromise(
    `mdfind -onlyin ~/Applications -onlyin /Applications 'kMDItemContentType == "com.apple.application-bundle" && kMDItemAppStoreIsAppleSigned != 1' -attr kMDItemFSName -attr kMDItemCFBundleIdentifier -attr kMDItemContentModificationDate`,
  );
...
export function useSortedApplications(search?: string) {
  ...
  const results = fuzzysort.go(search || "", applications || [], {
    key: "name",
  });
